...
## Maze features (`worlds/maze_world.world`, generated by `scripts/gen_maze_world.py`)

- **Grid:** 7×7 cells, **0.5 m** per cell; walls **0.015 m** thick, **0.2 m** high.
- **Top entrances:** gaps in the south boundary at columns 1, 4, and 7 (`y=0`).
- **Exit:** gaps in the north-west boundary (`y=7`, `x=0` and `x=1`) near the objects.
- **Objects:** two cubes (red + blue, 0.02 × 0.03 × 0.20 m) in cell **(5, 1)**.
- Regenerate after editing `h` / `v` in the script: `python3 scripts/gen_maze_world.py`
